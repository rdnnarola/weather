...
const Main = () => {
  ...
  const getWeatherForecast = () => {
    axios
      .get(
        `http://api.openweathermap.org/data/2.5/forecast?id=${city}&appid=${API_KEY}`
      )
      .then((res) => {
        let maxRain = { date: "", value: 0 };
        let maxSnow = { date: "", value: 0 };
        let temp: any = {};
        ...
        res.data.list.forEach((oneDayData: any) => {
          ...
          if (typeof temp[date] === "undefined") {
            ...
            if (oneDayData.snow) {
              if (maxSnow.value <= oneDayData.snow["3h"]) {
                maxSnow = {
                  date,
